<?php

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', 'Auth\LoginController@showLoginForm');
Auth::routes();

Route::get('/home', 'HomeController@index')->name('home');
Route::resource('schedules', 'ScheduleController');
Route::post('schedules/store', 'ScheduleController@store');

Route::resource('companies', 'CompanyController');
Route::get('/companies/{company}/createSchedule', 'CompanyController@createSchedule')->name('companies.createSchedule');
Route::post('storeSchedule', 'CompanyController@storeSchedule')->name('companies.storeSchedule');

Route::get('allAppUser', 'UsersController@allAppUsers')->name('users.allAppUser');

Route::resource('users', 'UsersController');

Route::resource('deliveries', 'DeliveryController')->middleware('admin.courier');

